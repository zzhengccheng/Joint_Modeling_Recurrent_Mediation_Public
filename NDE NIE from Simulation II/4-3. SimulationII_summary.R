...
for (i in 1:200){
  myBest=NULL
  myest1=read.csv(paste("estII_lres",as.character(i),".csv",sep=""))
  myest=c(myest1$NDE[-c(9,18)],myest1$NIE[-c(9,18)])
  for (batch in 1:5){
    
    skip_to_next <- FALSE
    tryCatch({myBest_temp=read.csv(paste("BootII_lres",as.character(i),"batch=",as.character(batch),".csv",sep=""))}, error = function(e) { skip_to_next <<- TRUE})
...
###Save the result###
write.csv(rbind(truevalue1,bias,ese,sd,cr),"sim2res_log.csv")
