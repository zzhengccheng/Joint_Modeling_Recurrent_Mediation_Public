...
est=read.table(paste("estIIlog",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1)
...
est_m=c(est$log_lam_m1,est$log_lam_m2,est$log_lam_m3,est$log_lam_m4,
        ...
        est$log_lam_y4,
        est$betax,est$betaz,est$etaz,
        est$etax,est$etam,est$delta1,est$delta2,est$log_varc)

####read in estimated variance by simseed####

estvar=read.table(paste("covIIlog",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1,na.strings = ".")
...
write.csv(myBest,paste("BootII_lres",as.character(simseed+1),"batch=", as.character(bootbatch),".csv",sep=""),row.names=FALSE)
}
