...
data=read.csv(paste("sim3_alldata",as.character(simseed+1),".csv",sep=""),header=TRUE)
...
est=read.table(paste("estIIIlog",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1)
...
est_m=c(est$log_lam_m1,est$log_lam_m2,est$log_lam_m3,est$log_lam_m4,
        ...
        est$etax,est$etam,est$delta1,est$delta2,est$log_varc)

####read in estimated variance by simseed####

estvar=read.table(paste("covIIIlog",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1,na.strings = ".")
...
myest=mymed(X,n,est,tseq,mod=2,B)
...
for (m in 1:M){
	...
	mymed_m=mymed(X,n,est,tseq,mod=2,B)
...
write.csv(myBest,paste("BootIII_lres",as.character(simseed+1),"batch=", as.character(bootbatch),".csv",sep=""),row.names=FALSE)
